...
def load_data(file_path):
    ...
    try:
        with open(file_path, 'r') as file:
            for line in file:
                try:
                    ...
                    if len(values) < 4:
                        print(f"Skipping invalid line (less than 4 values): {line.strip()}")
                        continue
                    t, m, x, y = map(float, values[:4])
                    times.append(t)
                    magnitudes.append(m)
                    xs.append(x)
                    ys.append(y)
        ...
        return np.array(times), np.array(magnitudes), np.array(xs), np.array(ys)
...
def complete_locations_sequential_omori(
    observed_times,
    observed_magnitudes,
    observed_xlocs,
    observed_ylocs,
    ...
    p=1.02,
    c=0.005,
    location_std_dev=0.05
):
    """
    Completes missing aftershock locations sequentially using all historical events 
    and precise Omori-Utsu law weights.

    Args:
        observed_times, observed_magnitudes, observed_xlocs, observed_ylocs: 
            Arrays of observed historical earthquake data.
        missing_times, missing_magnitudes: 
            Time and magnitude of the data points with missing locations.
        p (float): 
            The p-value for the Omori-Utsu law, controlling the decay rate. 
            Typically greater than 1.
        c (float): 
            The c-value (time offset constant) for the Omori-Utsu law.
        location_std_dev (float): 
            The standard deviation for each Gaussian kernel in the GMM.

    Returns:
        completed_points (np.array): 
            A complete array containing time, magnitude, and the imputed X, Y 
            coordinates (shape: [n_missing, 4]).
    """
    # 1. Prepare and merge data
    # Add an 'is_observed' flag to the observed data (1 for observed)
    observed_data = np.stack([
        observed_times, observed_magnitudes, observed_xlocs, observed_ylocs, np.ones(len(observed_times))
    ], axis=1)

    # Add a flag for missing data (0 for to-be-imputed) and use NaN as a placeholder for coordinates
    missing_data = np.stack([
        missing_times, missing_magnitudes, np.full(len(missing_times), np.nan), np.full(len(missing_times), np.nan), np.zeros(len(missing_times))
    ], axis=1)
    
    # Combine all events
    all_events = np.vstack([observed_data, missing_data])

    # 2. Sort all events chronologically (from earliest to latest)
    sort_indices = np.argsort(all_events[:, 0])
    sorted_events = all_events[sort_indices]

    # 3. Iterate through the sorted data and perform sequential imputation
    for i in range(len(sorted_events)):
        # If it's an observed point, skip it
        if sorted_events[i, 4] == 1:
            continue

        # If it's an imputation point, start the calculation
        # a. Extract the time of the current imputation point and all preceding historical events
        t_current = sorted_events[i, 0]
        history = sorted_events[:i, :]
        
        # Check if there is any history; cannot impute if not.
        if len(history) == 0:
            print(f"Warning: Imputation point at index {i} (time: {t_current}) has no preceding history. Skipping this point.")
            continue

        # b. Extract the times and locations of historical events
        t_past = history[:, 0]
        locs_past = history[:, 2:4]  # Extract historical X, Y coordinates

        # c. Calculate the time differences
        delta_t = t_current - t_past
        
        # Ensure time differences are positive to avoid numerical issues
        delta_t[delta_t <= 0] = 1e-9 

        # d. Calculate weights based on the Omori-Utsu formula
        # Weight w is proportional to (1 + Δt/c)^(-p)
        weights = (1 + delta_t / c)**(-p)
        
        # e. Normalize the weights
        sum_weights = np.sum(weights)
        if sum_weights == 0:
             # If all weights are zero (highly unlikely), use uniform weights
            normalized_weights = np.full(len(weights), 1.0 / len(weights))
        else:
            normalized_weights = weights / sum_weights

        # f. Sample a new location from the GMM
        # Step 1: Choose a historical event as the Gaussian kernel based on the normalized weights
        chosen_component_index = np.random.choice(
            len(history),
            p=normalized_weights
        )
        mean_location = locs_past[chosen_component_index]

        # Step 2: Sample from the chosen Gaussian distribution
        covariance_matrix = np.array([
            [location_std_dev**2, 0],
            [0, location_std_dev**2]
        ])
        sampled_location = np.random.multivariate_normal(
            mean=mean_location,
            cov=covariance_matrix
        )

        # g. Fill in the generated coordinates for the current event, making it available for subsequent imputations
        sorted_events[i, 2] = sampled_location[0]
        sorted_events[i, 3] = sampled_location[1]

    # Return an array containing all events with imputed locations
...
class BEPITApp:
    ...
    def load_data_only(self):
        ...
        if not self.file_path:
            return
        try:
            self.times, self.magnitudes, self.xlocs, self.ylocs = load_data(self.file_path)
            self.points = None
            self.final_poly = None
            self.intersection_polygons = []
            self.union_polygons = []
    ...
    def save_replenished_data(self):
        ...
        if not save_path:
            return
        try:
            all_points = complete_locations_sequential_omori(self.times, self.magnitudes, self.xlocs, self.ylocs, self.missing_points[:, 0], self.missing_points[:, 1])
            np.savetxt(save_path, all_points, fmt='%.6f %.6f %.6f %.6f %d', header="Time Magnitude X Y Flag")
